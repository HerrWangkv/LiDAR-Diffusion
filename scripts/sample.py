...
def convsample_ddim(model, steps, shape, eta=1.0, verbose=False):
    ...
    samples, intermediates = ddim.sample(
        steps, batch_size=bs, shape=shape, eta=eta, verbose=verbose, disable_tqdm=True
    )
    ...
    with model.ema_scope("Plotting"):
        ...
        if vanilla:
            sample, progrow = convsample(
                model, shape, make_prog_row=True, verbose=verbose
            )
        else:
            sample, intermediates = convsample_ddim(
                model, custom_steps, shape, eta, verbose
            )
        t1 = time.time()
    ...
    if model.cond_stage_model is None:
        ...
        for _ in trange(
            ...
            logs = make_convolutional_sample(
                model, batch_size, image_size, vanilla, custom_steps, eta, verbose
            )
            n_saved = save_logs(
                logs,
                imglogdir,
...
def save_logs(
    logs, imglogdir, pcdlogdir, n_saved=0, key="samples", np_path=None, config=None
):
    for k in logs:
        if k == key:
            batch = logs[key]
