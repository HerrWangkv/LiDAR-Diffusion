...
class DatasetBase(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        if self.log_scale:
            self.depth_thresh = (
                (np.log2(1.0 / 255.0 + 1) / self.depth_scale) * 2.0 - 1 + 1e-6
            )
        else:
            self.depth_thresh = (1.0 / 255.0 / self.depth_scale) * 2.0 - 1 + 1e-6
        self.return_pcd = return_pcd

        if degradation is not None and scale_factors is not None:
            scaled_img_size = (
                int(self.img_size[0] / scale_factors[0]),
                int(self.img_size[1] / scale_factors[1]),
            )
            degradation_fn = {
            ...
            self.degradation_transform = partial(
                TF.resize, size=scaled_img_size, interpolation=degradation_fn
            )
        else:
        ...
        self.anno_transform = (
            get_anno_transform(aug_config, split)
            if condition_key in ["bbox", "center"]
            else None
        )
        self.view_transform = (
            get_camera_transform(aug_config, split)
            if condition_key in ["camera"]
            else None
        )
