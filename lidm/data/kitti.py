...
KITTI_TRAIN_SET = SEM_KITTI_TRAIN_SET + [
    "11",
    "12",
    "13",
    "14",
...
KITTI360_TRAIN_SET = ["00", "02", "04", "05", "06", "07", "09", "10"] + [
    "08"
]  # partial test data at '02' sequence
CAM_KITTI360_TRAIN_SET = [
    "00",
    "04",
    "05",
    "06",
...
class SemanticKITTIBase(KITTIBase):
    ...
    def load_semantic_map(self, path, pcd):
        ...
        proj_range, sem_map = pcd2range(
            pcd, self.img_size, self.fov, self.depth_range, labels=labels
        )
        ...
        if self.filtered_map_cats is not None:
            sem_map[np.isin(sem_map, self.filtered_map_cats)] = (
                0  # set filtered category as noise
            )
...
class SemanticKITTITrain(SemanticKITTIBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="/hkfs/work/workspace_haic/scratch/xw2723-lidar/dataset/SemanticKITTI", split="train", **kwargs)


class SemanticKITTIValidation(SemanticKITTIBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="/hkfs/work/workspace_haic/scratch/xw2723-lidar/dataset/SemanticKITTI", split="val", **kwargs)
...
class KITTI360Base(KITTIBase):
    def __init__(self, split_per_view=None, **kwargs):
        ...
        if self.condition_key == "camera":
            assert (
                self.split_per_view is not None
            ), "For camera-to-lidar, need to specify split_per_view"
        ...
        for seq_id in seq_list:
            self.data.extend(
                glob.glob(
                    os.path.join(
                        self.data_root,
                        f"data_3d_raw/2013_05_28_drive_00{seq_id}_sync/velodyne_points/data/*.bin",
                    )
                )
            )
    ...
    def random_drop_camera(self, camera_list):
        if np.random.rand() < self.aug_config["camera_drop"] and self.split == "train":
            camera_list = [
                np.zeros_like(c) if i != len(camera_list) // 2 else c
                for i, c in enumerate(camera_list)
            ]  # keep the middle view only
        return camera_list

    def load_camera(self, path):
        camera_path = (
            path.replace("data_3d_raw", "data_2d_camera")
            .replace("velodyne_points/data", "image_00/data_rect")
            .replace(".bin", ".png")
        )
        camera = np.array(Image.open(camera_path)).astype(np.float32) / 255.0
        camera = camera.transpose(2, 0, 1)
        if self.view_transform:
            camera = self.view_transform(camera)
        camera_list = np.split(
            camera, self.split_per_view, axis=2
        )  # split into n chunks as different views
...
class KITTI360Train(KITTI360Base):
    def __init__(self, **kwargs):
        super().__init__(data_root="/hkfs/work/workspace_haic/scratch/xw2723-lidar/dataset/KITTI-360", split="train", **kwargs)


class KITTI360Validation(KITTI360Base):
    def __init__(self, **kwargs):
        super().__init__(data_root="/hkfs/work/workspace_haic/scratch/xw2723-lidar/dataset/KITTI-360", split="val", **kwargs)
...
class AnnotatedKITTI360Base(Annotated3DObjectsDataset, KITTI360Base):
    ...
    def prepare_data(self):
        ...
        self.data = [
            p for p in self.data if "2013_05_28_drive_0008_sync" not in p
        ]  # remove unlabeled sequence 08
        seq_list = eval("KITTI360_%s_SET" % self.split.upper())
        for seq_id in seq_list:
            if seq_id != "08":
                xml_path = os.path.join(
                    self.data_root,
                    f"data_3d_bboxes/train/2013_05_28_drive_00{seq_id}_sync.xml",
                )
    ...
    def load_annotation(self, path):
        ...
        if self.condition_key == "bbox":
            ...
            points = (verts_list[0] + verts_list[6]) / 2.0
        else:
            raise NotImplementedError
    ...
    def __getitem__(self, idx):
        ...
        proj_bbox_points, proj_bbox_labels = pcd2coord2d(
            bbox_points, self.fov, self.depth_range, labels=bbox_labels
        )
        builder = self.conditional_builders[self.condition_key]
        if self.condition_key == "bbox":
            proj_bbox_points = corners_3d_to_2d(proj_bbox_points)
            annotations = [
                Annotation(bbox=bbox.flatten(), category_id=label)
                for bbox, label in zip(proj_bbox_points, proj_bbox_labels)
            ]
        else:
            annotations = [
                Annotation(center=center, category_id=label)
                for center, label in zip(proj_bbox_points, proj_bbox_labels)
            ]
...
class AnnotatedKITTI360Train(AnnotatedKITTI360Base):
    def __init__(self, **kwargs):
        super().__init__(
            data_root="/hkfs/work/workspace_haic/scratch/xw2723-lidar/dataset/KITTI-360", split="train", cats=BBOX_CATS, **kwargs
        )
        ...
        for seq_id in eval("KITTI360_%s_SET" % self.split.upper()):
            self.data.extend(
                glob.glob(
                    os.path.join(
                        self.data_root,
                        f"KITTI-360/data_3d_raw/2013_05_28_drive_00{seq_id}_sync/velodyne_points/data/*.bin",
                    )
                )
            )
        ...
        for seq_id in eval("KITTI_%s_SET" % self.split.upper()):
            self.data.extend(
                glob.glob(
                    os.path.join(
                        self.data_root,
                        f"SemanticKITTI/dataset/sequences/{seq_id}/velodyne/*.bin",
                    )
                )
            )
...
class KITTIImageTrain(KITTIImageBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="/hkfs/work/workspace_haic/scratch/xw2723-lidar/dataset", split="train", **kwargs)


class KITTIImageValidation(KITTIImageBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="/hkfs/work/workspace_haic/scratch/xw2723-lidar/dataset", split="val", **kwargs)
