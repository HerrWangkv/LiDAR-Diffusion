...
from .conditional_builder.objects_center_points import (
...
class Annotated3DObjectsDataset(Dataset):
    def __init__(
        ...
        num_beams: int,
        cats: List[str],
        cat_blacklist: Optional[List[str]] = None,
        **kwargs
    ):
        ...
        self.categories = (
            [c for c in cats if c not in cat_blacklist]
            if cat_blacklist is not None
            else cats
        )
