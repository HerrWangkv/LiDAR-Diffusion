...
def sparse_quantize(
    ...
    *,
    ...
    assert isinstance(voxel_size, tuple) and len(voxel_size) in [
        2,
        3,
    ]  # support 2D and 3D coordinates only
...
def pcd2voxel(pcd):
    ...
    feat = np.concatenate(
        (pcd, -np.ones((pcd.shape[0], 1))), axis=1
    )  # -1 for remission placeholder
    _, inds, inverse_map = sparse_quantize(
        pcd_voxel, 1, return_index=True, return_inverse=True
    )
...
def pcd2voxel_full(data_type, *args):
    ...
    for data in args:
        ...
        for pcd in data:
            ...
            _, indices, inverse_map = sparse_quantize(
                pcd_voxel, 1, return_index=True, return_inverse=True
            )
...
def pcd2bev_sum(data_type, *args, voxel_size=VOXEL_SIZE):
    ...
    for data in args:
        ...
        for pcd in data:
            ...
            # summation
            volume_sum[pcd_voxel[:, 0], pcd_voxel[:, 1]] += 1.0
        output += (volume_sum,)
    return output
...
def preprocess_pcd(pcd, **kwargs):
    depth = np.linalg.norm(pcd, 2, axis=1)
    mask = np.logical_and(
        depth > kwargs["depth_range"][0], depth < kwargs["depth_range"][1]
    )
    pcd = pcd[mask]
    return pcd
    ...
    for b_idx in range(batch_indices.max() + 1):
        ...
        # avg on sectors
            ...
            for i in range(NUM_SECTORS):
                sector_indices = torch.where(
                    (angle >= sector_range[i]) & (angle < sector_range[i + 1])
                )[0]
                sector_logits = logits[sector_indices].mean(0)
                sector_logits = torch.nan_to_num(sector_logits, 0.0)
        ...
        # avg by depth
            ...
            sector_range = torch.linspace(
                kwargs["depth_range"][0] + 3, kwargs["depth_range"][1], NUM_SECTORS + 1
            )
            sector_range[0] = 0.0
            logits_list = []
            for i in range(NUM_SECTORS):
                sector_indices = torch.where(
                    (bev_depth >= sector_range[i]) & (bev_depth < sector_range[i + 1])
                )[0]
                sector_logits = logits[sector_indices].mean(0)
                sector_logits = torch.nan_to_num(sector_logits, 0.0)
                logits_list.append(sector_logits)
...
def compute_logits(data_type, modality, *args):
    ...
    for data in args:
        ...
        for i in range(math.ceil(len(data) / bs)):
            ...
            if is_voxel:
                batch = [
                    pcd2voxel(preprocess_pcd(pcd, **dataset_config)) for pcd in batch
                ]
                batch = sparse_collate_fn(batch)
                batch = {
                    k: (
                        v.cuda()
                        if isinstance(v, (torch.Tensor, SparseTensor, PointTensor))
                        else v
                    )
                ...
                with torch.no_grad():
                    batch_out = model(
                        batch, return_final_logits=True, agg_type=AGG_TYPE
                    )
...
def compute_pairwise_cd_batch(reference, samples):
    ...
    reference = torch.from_numpy(
        np.vstack(
            [
                reference,
                np.ones((max_len - reference.shape[0], ndim), dtype=np.float32) * 1e6,
            ]
        )
    ).cuda()
    samples = [
        np.vstack([s, np.ones((max_len - s.shape[0], ndim), dtype=np.float32) * 1e6])
        for s in samples
    ]
    ...
    for i in range(samples.shape[0]):
        ...
        dist = (dist1[:len1].mean() + dist2[:len2].mean()) / 2.0
