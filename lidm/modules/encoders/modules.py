...
class SpatialRescaler(nn.Module):
    def __init__(
        ...
        self.interpolator = partial(
            torch.nn.functional.interpolate, mode=method, align_corners=True
        )
...
class FrozenClipMultiTextEmbedder(FrozenCLIPTextEmbedder):
    ...
    def encode(self, text):
        ...
        if not self.apply_all:
            new_z = torch.zeros(
                *z.shape[:2], z.shape[2] * self.num_views, device=z.device
            )
            new_z[
                :,
                :,
...
class FrozenClipMultiImageEmbedder(FrozenClipImageEmbedder):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.view_embedding = nn.Parameter(
            img_dim**-0.5 * torch.randn((1, num_views * split_per_view, img_dim))
        )
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            img_feats = [
                self.model.encode_image(self.preprocess(img))[:, None] for img in x
            ]
...
class FrozenClipImagePatchEmbedder(nn.Module):
    ...
    def __init__(
        ...
        self.view_embedding = nn.Parameter(
            img_dim**-0.5 * torch.randn((1, num_views * split_per_view, 1, img_dim))
        )
    ...
    def forward(self, x):
        # x is assumed to be in range [0,1]
        img_feats = [
            self.encode_image_patch(self.preprocess(img))[:, None] for img in x
        ]
        x = torch.cat(img_feats, 1).float() + self.view_embedding
