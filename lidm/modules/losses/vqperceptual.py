...
from .discriminator import (
    NLayerDiscriminator,
    LiDARNLayerDiscriminator,
    LiDARNLayerDiscriminatorV2,
)
from .perceptual import PerceptualLoss

VERSION2DISC = {
    "v0": NLayerDiscriminator,
    "v1": LiDARNLayerDiscriminator,
    "v2": LiDARNLayerDiscriminatorV2,
}
...
class VQGeoLPIPSWithDiscriminator(nn.Module):
    def __init__(
        ...
        perceptual_type="rangenet_final",
        dataset_config=dict(),
    ):
        super().__init__()
        ...
        if perceptual_factor > 0.0:
            print(f"{self.__class__.__name__}: Running with LPIPS.")
            self.perceptual_loss = PerceptualLoss(
                perceptual_type, dataset_config.depth_scale, dataset_config.log_scale
            ).eval()
        ...
        self.geometry_converter = GeoConverter(
            curve_length, False, dataset_config
        )  # force converting xyz output
    ...
    def forward(
        ...
        if self.mask_factor > 0 and masks is not None:
            pixel_rec_loss = self.pixel_loss(
                inputs.contiguous(), reconstructions[:, 0:1].contiguous()
            )
            mask_rec_loss = (
                self.pixel_loss(
                    masks.contiguous(), reconstructions[:, 1:2].contiguous()
                )
                * self.mask_factor
            )
        else:
        ...
        if self.geo_factor > 0:
            geo_rec_loss = (
                self.geo_loss(input_coord[:, :2], rec_coord[:, :2]) * self.geo_factor
            )
        else:
        ...
        if self.perceptual_factor > 0:
            perceptual_loss = (
                self.perceptual_loss(
                    (inputs.contiguous(), input_coord),
                    (reconstructions[:, 0:1].contiguous(), rec_coord),
                )
        ...
        rec_loss = (
            pixel_rec_loss + mask_rec_loss + geo_rec_loss + perceptual_loss
        ) / self.rec_scale
        nll_loss = rec_loss
        ...
        ############# GAN #############
        disc_factor = (
            0.0 if global_step > self.discriminator_iter_start else self.disc_factor
        )
        # update generator (input: img, mask, coord, [cond])
        if optimizer_idx == 0:
            disc_recons = reconstructions.contiguous()
            if self.geo_factor > 0:
                disc_recons = torch.cat(
                    (disc_recons, rec_coord[:, :2].contiguous()), dim=1
                )
        ...
        if optimizer_idx == 1:
            disc_inputs, disc_recons = (
                inputs.contiguous().detach(),
                reconstructions.contiguous().detach(),
            )
            if self.mask_factor > 0 and masks is not None:
                disc_inputs = torch.cat(
                    (disc_inputs, masks.contiguous().detach()), dim=1
                )
            if self.geo_factor > 0:
                disc_inputs = torch.cat(
                    (disc_inputs, input_coord[:, :2].contiguous()), dim=1
                )
                disc_recons = torch.cat(
                    (disc_recons, rec_coord[:, :2].contiguous()), dim=1
                )
