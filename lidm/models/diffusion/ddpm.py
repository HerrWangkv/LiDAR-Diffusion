    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if (
                ...
                patch_limits_tknzd = [
                    torch.LongTensor(self.bbox_tokenizer.crop_encoder(bbox))[None].to(
                        self.device
                    )
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        if ddim:
            ...
            samples, intermediates = ddim_sampler.sample(
                ddim_steps, batch_size, shape, cond, verbose=self.verbose, **kwargs
            )
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                if isinstance(batch["camera"], list):
                    ...
                    xc = xc.reshape(*xc.shape[:3], -1) * 2.0 - 1.0
                log["conditioning"] = xc
            elif isimage(xc):
                log["conditioning"] = xc
            if ismap(xc):
                ...
                label2rgb = (
                    torch.from_numpy(dset.label2rgb).to(self.device) / 127.5 - 1.0
                )
                # log["original_conditioning"] = self.to_rgb(xc)
                log["original_conditioning"] = label2rgb[xc.argmax(1)].permute(
                    0, 3, 1, 2
                )
...
class Layout2ImgDiffusion(LatentDiffusion):
    def __init__(self, cond_stage_key, *args, **kwargs):
        assert cond_stage_key in [
            "bbox",
            "center",
        ], 'Layout2ImgDiffusion only for cond_stage_key="bbox" or "center"'
