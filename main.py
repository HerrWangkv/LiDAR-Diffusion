...
class ImageLogger(Callback):
    ...
    def log_local(self, save_dir, split, images, global_step, current_epoch, batch_idx):
        ...
        for k in images:
            ...
            img_name = "{}-{:06}_e-{:06}_b-{:06}.png".format(
            ...
            if isdepth(imgs):
                for i, img in enumerate(imgs[:2]):
                    ...
                    pcd_name = "{}_pcd_{:02}_gs-{:06}_e-{:06}_b-{:06}.txt".format(
                        k, i, global_step, current_epoch, batch_idx
                    )
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (
            self.check_frequency(check_idx, split)  # batch_idx % self.batch_freq == 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        if "autoencoder" in config.model.target:
            config.model.params.lossconfig.params.dataset_config = (
                config.data.params.dataset
            )
